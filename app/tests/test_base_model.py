...
def sample_file_path():
    ...
    return "/home/yannosh/ium/ium/app/input/monthly_listening_v3.jsonl" 

def test_train_base_model(sample_file_path):
    response = requests.post(
        f"{BASE_URL}/base_model/train",
        json={"file_path": sample_file_path}
    )
    assert response.status_code == 200
    assert "Base model trained successfully" in response.json().get("message", "")

def test_get_mae():
    response = requests.get(f"{BASE_URL}/base_model/mae")
    assert response.status_code == 200
    assert "mae" in response.json()
    assert isinstance(response.json()["mae"], float)

def test_get_predicted_values():
    response = requests.get(f"{BASE_URL}/base_model/predict")
    assert response.status_code == 200
    data = response.json()
...
def test_get_feature_importances():
    response = requests.get(f"{BASE_URL}/base_model/importance_features")
    assert response.status_code == 200
